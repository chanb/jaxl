...
class ImplicitInContextSupervisedTransformer(Model):
    ...
    def _make_get_positional_encoding(
        ...
        if input_output_same_encoding:
            ...
            def apply_positional_encoding(
                params, queries, input_embedding, output_embedding, **kwargs
            ):
                ...
                output_embedding = self.positional_encoding.apply(
                    params[CONST_POSITIONAL_ENCODING],
                    output_embedding,
                    **kwargs,
                )
                ...
                stacked_inputs = jnp.concatenate(
                    (input_embedding, output_embedding), axis=-1
    ...
    def make_tokenize(
        ...
        def tokenize(
            ...
            **kwargs,
            ...
            :param proposed_outputs: the proposed output
            :type params: Union[optax.Params, Dict[str, Any]]
            :type queries: chex.Array
            :type contexts: Dict[str, chex.Array]
            :type proposed_outputs: chex.Array
            ...
            output_embedding, output_updates = self.output_tokenizer.apply(
        ...
        def get_latent(
            ...
            **kwargs,
            ...
            stacked_inputs, _, token_updates = self.tokenize(
                params, queries, contexts, proposed_outputs, eval, **kwargs
            )
    ...
    def make_forward(self, query_pred_only: bool) -> Callable[
        ...
        def process_prediction(preds):
            return np.sum(preds[:, -2] * preds[:, -1], axis=-1)
...
class ImplicitNoTokenizerICSupervisedTransformer(
    ...
    def make_tokenize(
        ...
        def tokenize(
            ...
            **kwargs,
            ...
            output_embedding, _, output_updates = self.output_tokenizer.forward(
                params[CONST_OUTPUT_TOKENIZER],
                jnp.concatenate(
                    (
                        contexts[CONST_CONTEXT_OUTPUT],
                        proposed_outputs[:, None],
                    ),
                    axis=1,
                ),
                None,

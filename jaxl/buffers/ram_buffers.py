...
- MemoryEfficientNumPyBuffer assumes that the next observation (or state) 
  is followed by the current observation. Thus, we save memory by removing
  the buffer for storing the next states.
- TrajectoryNumPyBuffer treats a single trajectory as a sample. This supports

...
def main(config):
    ...
    with open(
        os.path.join(
            config.out_dir,
            f"hyperparameters-pretrain_mtbc-{config.hyperparam_set}-{config.exp_name}_{control_mode}-offset_{config.offset}.pkl",
        ),
        "wb",
    ) as f:
    ...
    for idx, hyperparams in enumerate(itertools.product(*hyperparamss)):
        ...
        variant_name = "num_tasks_{}".format(num_tasks)
        ...
        for task_i in range(num_tasks):
            template["learner_config"]["buffer_configs"].append(
                {
                    "load_buffer": dataset_paths[task_i + config.offset],
                    "buffer_type": "default",
            ...
            dataset_info = dataset_paths[task_i + config.offset].split(".")
            env_config = {
                "env_type": "gym",
        ...
        variant = "model_seed_{}-env_seed_{}".format(model_seed, env_seed)
        template["logging_config"]["experiment_name"] = variant
        template["logging_config"]["save_path"] = "{}".format(curr_run_dir)
    ...
    dat_path = os.path.join(
        f"./export-pretrain-mtbc-{config.hyperparam_set}-{config.exp_name}_{control_mode}-env_seed_{env_seed}.dat"
    )
    ...
    with open(
        os.path.join(
            f"./run_all-pretrain-mtbc-{config.hyperparam_set}-{config.exp_name}_{control_mode}-offset_{config.offset}.sh"
        ),
        "w+",
    ) as f:

...
for env_name, env_config in envs:
    ...
    for (subsampling, seed) in product(env_config["subsamplings"], seeds):
        ...
        template["learner_config"]["buffer_config"]["load_buffer"] = "{}-subsampling_{}.gzip".format(env_config["expert_data_prefixes"], subsampling)
        template["learner_config"]["seeds"] = {
            "model_seed": seed,
            "buffer_seed": seed,
        }

        out_path = "{}-subsampling_{}-seed_{}".format(env_name, subsampling, seed)

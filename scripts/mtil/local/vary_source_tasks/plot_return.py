...
ref_name = "results-finetune_mtbc_main"
experiment_name = "results-finetune_mtbc-vary_source_tasks"
...
for exp_i, exp_name in enumerate([ref_name, experiment_name]):
    curr_exp = f"{exp_name}"
    ...
    if os.path.isfile(f"{save_path}/results.pkl"):
        ...
        for eval_path, _, filenames in os.walk(exp_dir):
...
for env_name in env_names:
    ...
    for ax_i, env_seed in enumerate(results_per_experiment[experiment_name][env_name]):
        ...
        for exp_i, curr_exp in enumerate([ref_name, experiment_name]):
            res = results_per_experiment[curr_exp][env_name][env_seed]["mtbc"]

            if exp_i == 0:
                res = {"ref": res}

            for data_sample_seed in res:
                num_tasks, _, returns = list(zip(*res[data_sample_seed]))
                num_tasks = np.array(num_tasks)

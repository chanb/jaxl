...
bc_dir = f"/Users/chanb/research/personal/mtil_results/final_results/data/rebuttal/evaluations/{bc_name}"
...
for env_name in env_names:
    num_envs = len(results[env_name]) // len(experiment_name_suffixes)
    ...
    for ax_i, env_seed in enumerate(ref_result):
        ...
        ax.set_title("Variant {}".format(ax_i + 1))
        ax.set_ylim(0, 1.1)
        # ax.legend()
...
for env_i, env_name in enumerate(env_names):
    ...
    #         num_rows,
    #         num_cols,
    #         figsize=set_size(doc_width_pt, 0.5, (num_rows, num_cols), use_golden_ratio=False),
    #         layout="constrained",
    #     )
    ...
    for env_seed in ref_result:
        ...
        for suffix in experiment_name_suffixes:
            ...
            print(unique_num_tasks)
            for num_task in unique_num_tasks:
                mtbc_rets[num_source_data].setdefault(num_task, [])

                print(returns[num_tasks == num_task].shape)
                curr_num_task_rets = normalize(returns[num_tasks == num_task])
    ...
    for num_source_data in mtbc_rets:
        ...
        print(mtbc_rets[num_source_data][num_task])
        print(means, stds)
        ax.plot(
            unique_num_tasks,
            means,
...
# fig.suptitle("{} {}".format( map_env[env_name[0]], map_control[env_name[1]]))
fig.savefig(
    f"{save_plot_dir}/cartpole-include_target_task.pdf",
    format="pdf",
    bbox_inches="tight",
    dpi=600,
)

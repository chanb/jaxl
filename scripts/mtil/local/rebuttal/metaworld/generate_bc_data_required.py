...
expert_dataset = "/home/bryanpu1/projects/jaxl/scripts/mtil/local/rebuttal/metaworld/expert_data/test_metaworld-img_res_84-subsampling_490-scrambling_{}-num_samples_50000-seed_{}.gzip".format(
    scrambling, dataset_seed
)
bc_template = "/home/bryanpu1/projects/jaxl/scripts/mtil/local/rebuttal/metaworld/configs/bc_template.json"

save_path = (
    "/home/bryanpu1/projects/jaxl/scripts/mtil/local/rebuttal/metaworld/results/bc"
)
arch_name = "large_network"

config_out_path = "/home/bryanpu1/projects/jaxl/scripts/mtil/local/rebuttal/metaworld/configs/bc_data_ablation-{}".format(
    arch_name
)
script_out_path = "/home/bryanpu1/projects/jaxl/scripts/mtil/local/rebuttal/metaworld/bc_data_ablation-{}-scrambling_{}-seed_{}.sh".format(
    arch_name, scrambling, dataset_seed
)
...
for num_data in num_data_to_check:
    for model_seed in range(num_seeds):
        ...
        template["logging_config"]["save_path"] = os.path.join(
            save_path, f"scrambling_{scrambling}-dataset_seed_{dataset_seed}", arch_name
        )
        template["logging_config"][
            "experiment_name"
        ] = f"num_data_{num_data}-model_seed_{model_seed}"

        modify_network(template, arch_name)

        curr_config_path = os.path.join(
            config_out_path,
            "bc_data_ablation-num_data_{}-model_seed_{}-scrambling_{}-seed_{}.json".format(
                num_data, model_seed, scrambling, dataset_seed
            ),
        )

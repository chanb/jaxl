...
config_out_path = "/home/bryanpu1/projects/jaxl/scripts/mtil/local/rebuttal/metaworld/configs/pretrain_mtbc-{}".format(
    arch_name
)
...
for task_i, num_tasks in enumerate(num_taskss):
    script_out_path = os.path.join(
        script_out_dir, "pretrain_mtbc-{}-num_tasks_{}.sh".format(arch_name, num_tasks)
    )
    ...
    for buffer_seed in range(num_tasks):
        expert_dataset = os.path.join(
            expert_datasets_dir, "test_metaworld-seed_{}.gzip".format(buffer_seed)
        )
        curr_buffer_config = {
    ...
    for model_seed in range(num_runs):
        ...
        template["logging_config"]["save_path"] = os.path.join(
            save_path, arch_name, f"num_tasks_{num_tasks}"
        )
        ...
        curr_config_path = os.path.join(
            config_out_path,
            "pretrain_mtbc-model_seed_{}-num_tasks_{}.json".format(
                model_seed, num_tasks
            ),
        )

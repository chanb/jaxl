...
for exp_name, exp_runs in agg_result.items():
    for run_name, exp_run in exp_runs.items():
        ...
        curr_context_len = max(len(context_lens[-1]) - 1 for context_lens in exp_run["auxes"].values())
...
def context_len_exp_runs(exp_runs: dict, x_range: chex.Array):
    ...
    for run_i, (run_name, exp_run) in enumerate(exp_runs.items()):
        for eval_name, eval_result in exp_run["auxes"].items():
            curr_context_lens = [len_i for len_i in range(max_context_len) if len_i in eval_result[-1]]
            curr_accuracies = [eval_result[-1][len_i]["accuracy"] for len_i in curr_context_lens]
            curr_ratios = [eval_result[-1][len_i]["query_class_in_context_ratio"] for len_i in curr_context_lens]
